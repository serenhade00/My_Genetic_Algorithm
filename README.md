...
- <mark>생물체가 환경에 적응하면서 진화해가는 모습을 모방</mark>하여 **유전** 알고리즘으로 불린다.
...
* 결국, 세대를 거쳐가며 적합도가 높은(MSE 낮음) 해가 더 많이 선택되었고 그 우수한 해들끼리 교차, 변이 연산을 거치며 점점 더 좋은 해로 발전할 여지가 나왔다. 또한 그런 해들끼리 자손을 만들어 더 좋은 적합도를 가진해가 나오면 다음 선택 과정에서 그 해가 이제 부모가 되는것이다. 이런 경향을 통해 좋은 해가 선택되었다. 
* 정상적으로 선택, 교차 연산이 일어나고, 변이 연산은 낮은 확률 때문에 이 예제에서는 확인되지 않았다. 변이 연산의 확률이 높다면, 최적해에 잘 접근하고 있던 해가 갑자기 엉뚱한 해가 되버리는 일이 생길수도 있기에 확률을 조정했다. 
